...
import { updateModuleSettings, resetModuleSettings } from "../../actions";
            ...
            sectionInputs.push(
                section.inputs.map((input) => {
                    ...
                    if (
                        ...
                        this.props.moduleSettings[this.state.module.id].settings[
                        input.id
                        ] != undefined
                    ) {
                        // console.log("DONT SET VAUE SHJORT TEXT");
                        value =
                            this.props.moduleSettings[this.state.module.id].settings[input.id]
                                .value;
                    } else if (this.state.module.settings[input.id]?.value && input.type != "channel_select" && input.type != "role_select" && input.type != "multi_role_select" && input.type != "multi_channel_select") {
                    ...
                    if (input.show) {
                        ...
                        if (
                            ...
                            for (var key in this.props.moduleSettings[this.state.module.id]
                                .settings) {
                            ...
                            var sectionInputs = this.state.module.sections.find((section) =>
                                section.inputs.find((input) => input.id == show.id)
                            );

                            if (sectionInputs) {
                                var showInput = sectionInputs.inputs.find(
                                    (input) => input.id == show.id
                                );
            ...
            if (inputs.length > 0) {
                sections.push(
                    <section
                        ...
                        premiumRequired={
                            this.state.module.premium || section.premium == true
                                ? true
                                : false
                        }
                    >
            ...
            <Dashboard server_id={this.props.match.params.server_id}>
                ...
                <header className="bg-menu-color rounded-md flex px-5 py-8 items-center">
                    ...
                    <div className="ml-auto">
                        ...
                        {this.props.moduleSettings[this.state.module.id] !== undefined ? <Button className="btn btn-neutral mr-2" onClick={(e) => {
                                        ...
                                        this.state.module.sections.forEach((section) => {
                                            section.inputs.forEach((input) => {
                                                if (input.type.includes("multi")) {
                                                    var value = input.defaultValue != undefined ? input.defaultValue : [];
                                                    // Check if the value exists in the settings
                                                    if (this.state.module.settings[input.id]?.value && input.type != "channel_select" && input.type != "role_select" && input.type != "multi_role_select" && input.type != "multi_channel_select") {
                                                        value = this.state.module.settings[input.id]?.value;
                                                    }
                                                    ...
                                                    var value = input.defaultValue != undefined ? input.defaultValue : [];
                                                    // Check if the value exists in the settings
                                                    if (this.state.module.settings[input.id]?.value) {
                                                        value = this.state.module.settings[input.id]?.value;
                                                    }
                                                    moduleSettings[input.id] = {
                                                        value: value,
