...
export class RoleAdd extends Component {
  ...
  renderDropDown = () => {
    const items = [];
    if (
      this.props.server_data.loaded == false &&
      ...
      return (
        <div className="z-50 absolute w-[250px] h-[300px] bg-darkGray p-3 rounded-md top-[55px] left-0">
          <div className="role-adder-dropdown-main">
            ...
            <span>
              Click the refresh button above to reload your server data.
            </span>
          </div>
        </div>
      );
        ...
        this.props.server_data.roles.forEach((role) => {
          if (
            ...
            items.push(
              <div
                key={role.id}
                className="text-white text-sm p-3 hover:opacity-80 hover:cursor-pointer"
                onClick={(e) => {
                  this.addItem(role);
      ...
      return (
        <div className="z-50 absolute w-[250px] h-[300px] overflow-y-scroll bg-darkGray p-3 rounded-md top-[50px] left-0">
          <div className="role-adder-dropdown-main">{items}</div>
        </div>
      );
    }
  };
        ...
        items.push(
          <li className="badge badge-neutral" key={index}>
            <FontAwesomeIcon
              ...
              icon={faTimesCircle}
            />
            <span className="ml-[3px]">{item}</span>
          </li>
        );
      } else if (this.props.type == "channels") {
        if (
          (this.props.channelFilter == undefined &&
            (item.type == 0 || item.type == 4)) ||
          (this.props.channelFilter &&
            this.props.channelFilter.length > 0 &&
            this.props.channelFilter.includes(item.type))
        ) {
          items.push(
            <li className="badge badge-neutral" key={index}>
              <FontAwesomeIcon
                ...
                icon={faTimesCircle}
              />
              <span className="ml-[3px]">#{item.name}</span>
            </li>
          );
        }
      } else {
  ...
  render() {
    return (
      <div className="relative">
        ...
        <div
          className="input input-bordered flex items-center relative overflow-auto h-auto px-2 py-2"
          style={{}}
        >
            ...
            <li className="">
              <div className="flex">
                ...
                <PlusCircleIcon
                  ...
                  className="h-6 hover:cursor-pointer animate-pulse hover:opacity-80"
                />
                {/* </button> */}
              </div>
            </li>
